            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (string filename in files)
                {
                    if (filename.Contains(".mesh"))
                    {
                        ...
                        if (importdlg.textBox2.Text=="")
                        {
                            importdlg.textBox2.Text = "1";
                        }
                        MainOgreForm.Singleton.ModelGroup.AddModel(importdlg.textBox1.Text, name, importdlg.comboBox1.Text,float.Parse(importdlg.textBox2.Text));
                        break;
                    }
                }
                ...
                MessageBox.Show("模型导入成功！重启程序后生效","导入模型");
            }
        }
