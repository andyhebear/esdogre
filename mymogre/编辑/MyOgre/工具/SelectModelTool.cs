...
namespace MyOgre
{
    ...
    public class SelectModelTool:  AbstractTool
    {
        ...
        public bool SelectObject(Point pt)
        {
            ...
            {
                OgreView.Singleton.CurrentWgOperageNode.ShowBoundingBox = false;
            }
        ...
        {
            if (OgreView.Singleton.WRnode == null)
                return false;
            ...
            bool flag = true;
            //循环得到每个模型，和射线进行比较，得到和射线相交并且距屏幕最近的模型
            foreach (SceneNode node in OgreView.Singleton.WRnode.GetChildIterator())
            {
                //得到模型的边框值
                AxisAlignedBox aa = node._getWorldAABB();
                //是否和射线相交
                ...
                if (result.first)
                {
                    ...
                    if (flag)
                    {
                        flag = false;
                        dis = result.second;
                        OgreView.Singleton.CurrentWgOperageNode = node;
                    }
                    else//如果有相交的模型，比较那个模型距离屏幕最近
                    {
                        if (dis < result.second)
            ...
            if (!flag)
            {
                OgreView.Singleton.CurrentWgOperageNode.ShowBoundingBox = true;
                return true;
            }         
            //将操作对象置为空。
